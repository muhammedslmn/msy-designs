...
.band{
  ...
  padding:calc(var(--u) * 3.2) var(--pad) calc(var(--u) * 3.2 + env(safe-area-inset-bottom));
  height:calc(var(--bandh) + env(safe-area-inset-bottom));
...
.slide{
  ...
  padding:calc(var(--u) * 11 + env(safe-area-inset-top)) var(--pad) calc(var(--u) * 9);
...
.ar--xl{font-size:calc(var(--u) * 20 * var(--fit,1)); line-height:1.45}
.ar--lg{font-size:calc(var(--u) * 12 * var(--fit,1)); line-height:1.5}
.ar--sm{font-size:calc(var(--u) * 7 * var(--fit,1)); line-height:1.6; color:var(--gold); text-align:start}
.ar--root{
  font-size:calc(var(--u) * 17 * var(--fit,1)); line-height:1.5;
...
.panel{
  position:fixed; right:calc(var(--u) * 4); z-index:9;
  bottom:calc(var(--bandh) + var(--u) * 4 + env(safe-area-inset-bottom));
...
.recall{
  ...
  margin-bottom:calc(var(--bandh) + env(safe-area-inset-bottom));
...
(function () {
  ...
  function setBand(txt) {
    ...
    setTimeout(function () {
      ...
      band.style.setProperty('--bandfit', 1);
      var cs = getComputedStyle(band);
      var room = band.clientHeight - parseFloat(cs.paddingTop) - parseFloat(cs.paddingBottom);
      var have = bandt.scrollHeight;
      if (room > 0 && have > room) {
        band.style.setProperty('--bandfit', Math.max(0.6, room / have).toFixed(3));
      }
